      ...
      <section className="relative py-20 px-4 bg-background">
        <div className="max-w-[1200px] mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="mb-4">{t.features.title2}<br />{t.features.title2Br}</h2>
          </div>
          <div className="max-w-[900px] mx-auto">
            <div className="bg-white rounded-[20px] p-8 lg:p-12" style={{boxShadow: 'rgba(124, 58, 237, 0.1) 0px 4px 20px', opacity: 1, transform: 'none'}}>
              <div className="text-center mb-12">
                <h3 className="mb-3" style={{fontSize: '28px'}} dangerouslySetInnerHTML={{__html: t.features.compareTitle}} />
                <p className="text-base" style={{color: 'rgb(100, 116, 139)'}}>{t.features.compareDesc}</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <h4 className="font-semibold text-foreground mb-6 flex items-center gap-2">
                    ...
                    {t.features.othersLabel}
                  </h4>
                  {t.features.others.map((item, index) => (
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <p className="text-sm" style={{color: 'rgb(148, 163, 184)'}}>{item}</p>
                    </div>
                  ))}
                </div>
                <div className="space-y-4">
                  ...
                  {t.features.us.map((item, index) => (
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <p className="text-sm text-foreground"><span className="font-semibold">{item.bold}</span>{item.text}</p>
                    </div>
                  ))}
                </div>
              </div>
              <div className="mt-10 pt-8 border-t border-primary/10 text-center">
                <p className="text-sm font-semibold text-primary">{t.features.footer}</p>
              </div>
            </div>
          </div>
        </div>
